...
def build_dnn():
    ...
    ann.layers.append(SPConv(input_shape=(-1, 1, I1, I2), itype=dtype,
                             weight=W_conv1, bias=b_conv1,
                             strides=(1, 1), boundary='P'))
    ann.add_layer(functions.ReLU)
    ann.add_layer(functions.Pooling, mode='max',
                  kernel_shape=(2, 2), boundary='O')

    # second convolution layer
    W_conv2 = eta * typed_randn(dtype, (F2, F1, K1, K1))  # fout, fin, K1, K2
    b_conv2 = eta * typed_randn(dtype, (F2,))
    ann.add_layer(SPConv, weight=W_conv2, bias=b_conv2,
                  strides=(1, 1), boundary='P')
    ann.add_layer(functions.ReLU)
    ann.add_layer(functions.Pooling, mode='max',
                  kernel_shape=(2, 2), boundary='O')
    ...
    y_true = zeros(F4, dtype='float32')
    y_true[3] = 1
    assert(all(check_numdiff(ann, var_dict={
           'y_true': y_true, 'seed': 2}, eta_x=1e-3, tol=3e-2)))
...
def compute_gradient(weight_vec, info_dict):
    ...
    dnn.set_runtime_vars(
        {'y_true': info_dict['y_true'], 'seed': random.randint(1, 99999)})
    x = info_dict['x_batch']
...
def main(_):
    ...
    # info_dict['x_batch'] = asfortranarray(batch[0]).reshape(
    # [batch[0].shape[0],-1],order='F')   #add feature axis.
    info_dict['x_batch'] = asfortranarray(batch[0]).reshape(
        [batch[0].shape[0], 1, 28, 28], order='F')  # add feature axis.
    info_dict['y_true'] = asfortranarray(batch[1], dtype=batch[0].dtype)
    optimizer = RmsProp(wrt=var_vec, fprime=lambda x: compute_gradient(
        x, info_dict), step_rate=1e-3, decay=0.9, momentum=0.)
    # optimizer=GradientDescent(wrt=var_vec,
    # fprime=lambda x: compute_gradient(x,info_dict),
    # step_rate=1e-2,momentum=0.)
    # optimizer=Adam(wrt=var_vec,
    # fprime=lambda x: compute_gradient(x,info_dict),step_rate=1e-3)
    ...
    for k, info in enumerate(optimizer):
        ...
        batch = mnist.train.next_batch(50)
        # add feature axis.
        # info_dict['x_batch'] = asfortranarray(
        # batch[0]).reshape([batch[0].shape[0],-1],order='F')
        info_dict['x_batch'] = asfortranarray(batch[0]).reshape(
            [batch[0].shape[0], 1, 28, 28], order='F')  # add feature axis.
    ...
    ys = dnn.forward(asfortranarray(mnist.test.images).reshape(
        [-1, 1, 28, 28], order='F'), data_cache=data_cache)
