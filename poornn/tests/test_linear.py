...
def test_linear():
    ...
    sv = Linear((num_batch, nfi), 'float32', float32(
        weight), float32(cv.bias.data.numpy()))
    ...
    t3 = time.time()
    print("Elapse old = %s, new = %s, numpy = %s" %
          (t1 - t0, t2 - t1, t3 - t2))
    res1 = y1.data.numpy()
    res2 = y2
    ...
    assert_allclose(cv.bias.grad.data.numpy() /
                    bfactor, dbias / bfactor, atol=3e-3)
...
def test_linear_complex():
    ...
    sv = Linear((num_batch, dim_in), 'complex128',
                weight, bias, var_mask=(False, False))
    assert_allclose(sv.get_variables(), zeros(0))
...
def test_splinear():
    ...
    mat1 = sps.csr_matrix(
        (dwb1[:-dim_out], sv2.weight.indices, sv2.weight.indptr)).toarray()
    t2 = time.time()
    print("Elapse old = %s, new = %s" % (t1 - t0, t2 - t1))
    assert_(all(check_numdiff(sv, num_check=100)))
...
def test_linear1():
    ...
    cv = nn.Linear(nfi, nfo)
    weight = cv.weight.data.numpy()
    sv = Linear(input_shape=(nfi,), itype='float32', weight=float32(
        weight), bias=float32(cv.bias.data.numpy()))
    print("Testing forward for %s" % sv)
    xin_np = asfortranarray(ts.data.numpy())
    ntest = 5
    t0 = time.time()
