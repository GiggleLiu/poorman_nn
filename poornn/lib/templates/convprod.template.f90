module lib
    ...
    {%endif -%}
    subroutine forward_{{version}}{{dtype_token}}(x, y, csc_indptr, csc_indices, nnz, dim_in, dim_out, nfi)
        implicit none
    ...
    subroutine backward_{{version}}{{dtype_token}}(dy,x,y,dx,csc_indptr,csc_indices, nnz,dim_in,dim_out,nfi)
        implicit none
        integer,intent(in) :: nnz,dim_in,dim_out,nfi
        {{dtype}},intent(in) :: x(nfi, dim_in)
