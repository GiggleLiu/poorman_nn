...
def check_numdiff(layer, x=None, num_check=10,
                  eta_x=None, eta_w=None, tol=1e-3, var_dict={}):
    '''
    Random Numerical Differentiation check.

    Args:
        layer (Layer): the layer under check.
        x (ndarray|None, default=None): input data, \
randomly generated if is None.
        num_check (int, default=10): \
number of random derivative checks for both inputs and weights.
        eta_x (number, default=0.005 if float else 0.003+0.004j): \
small change on input, in order to obtain numerical difference.
        eta_w (number, default=0.005 if float else 0.003+0.004j): \
small change on weight, in order to obtain numerical difference.
        tol (float, default=1e-3): tolerence, relative difference \
allowed with respect to max(\|eta\|, \|gradient\|).
    ...
    if analytical == 4:
        print('Warning: Layer %s is not analytic, \
going on numdiff check!' % layer)
    elif analytical == 3 and layer.itype[:7] == 'complex':
        res = []
        for out_layer in ['Abs', 'Angle']:
            layer_i = get_complex_checker_net(layer, out_layer)
            res = res + check_numdiff(layer_i, x=x, num_check=num_check,
                                      eta_x=eta_x, eta_w=eta_w,
                                      tol=tol, var_dict=var_dict)
        return res
    ...
    if y.dtype != layer.otype:
        print('Warning: output data type not match, \
%s expected, but get %s! switch to debug mode:' % (
            layer.otype, y.dtype))
        pdb.set_trace()
    ...
    for i in range(num_check):
        ...
        if (analytical == 2 or analytical == 3) and\
                layer.itype[:7] == 'complex':
...
def generate_randx(layer):
    ...
    if input_shape is None:
        return typed_randn(input_dtype, np.random.randint(
            1, max_size + 1, np.random.randint(1, max_dim + 1)))
    input_shape = [n if n >= 0 else np.random.randint(
        1, max_size + 1) for n in input_shape]
...
def _check_input(layer, x):
    ...
    if x.ndim != len(layer.input_shape):
        raise ValueError('Dimension mismatch! layer %s, x %s, \
desire %s' % (
            layer, x.ndim, len(layer.input_shape)))
    for shape_i, xshape_i in zip(layer.input_shape, x.shape):
        if shape_i != -1 and shape_i != xshape_i:
            raise ValueError('Illegal Input shape! layer %s, \
x %s, desire %s' % (
                layer, x.shape, layer.input_shape))
...
def _check_output(layer, y):
    ...
    if y.ndim != len(layer.output_shape):
        raise ValueError('Dimension mismatch! y %s, desire %s' %
                         (y.ndim, len(layer.output_shape)))
    for shape_i, yshape_i in zip(layer.output_shape, y.shape):
        if shape_i != -1 and shape_i != yshape_i:
            raise ValueError('Illegal Output shape! y %s, desire %s' %
                             (y.shape, layer.output_shape))
...
def check_shape_match(shape_get, shape_desire):
    ...
    if len(shape_get) != len(shape_desire):
        raise ValueError('Dimension mismatch! get %s, desire %s' %
                         (len(shape_get), len(shape_desire)))
    ...
    for shape_get_i, shape_desire_i in zip(shape_get, shape_desire):
        if shape_desire_i == -1:
            ...
            if shape_desire_i != shape_get_i:
                raise ValueError('Shape Mismatch! get %s, desire %s' %
                                 (shape_get, shape_desire))
            else:
                shape.append(shape_get_i)
    return tuple(shape)
...
real output functions like %s.' % layer)
    if hasattr(layer, 'tags'):
        if layer.tags['analytical'] != 3:
            raise AnalyticityError(
                'Analyticity type 3 (not analytical only in complex plane) \
is expected, check your layer or tags.')
    abslayer = eval('functions.%s' % out_layer)(
        input_shape=layer.output_shape, itype=layer.otype)
